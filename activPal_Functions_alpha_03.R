...
#           a_4.4 - Integrated MVPA_Bout_detection_alpha.R
#                 - Settings for console output
#                 - BugFixed: External NW Tab could be used
#                 - Alternative Ways for Sleep Tab Creation implemented
#
#Troubleshooting: - Siesta could not be described in Sleep Tab but could potentialy create Problems for detection
#                 - Change of Daybreak Time doesnt work
#                 - NW Algorithsm are not tested
#                 - Potential Problems if Sleep Ends before Daybreak / if it starts after its fine
###########################################################################################################;
################ Set Global Options ##############################
  ...
  # creates Table with informations about the activity-levls for the day, domains and misc
{
  ...
  tab <- new.tab
  dates <- as.Date(tab[,2],tz=getOption("tz"))
  if(!is.null(NW.tab) && nrow(NW.tab) > 0)
  {
    new.tab <- NULL
    for(day in 1:length(unq.dates))
    {
...
FN_CPAHR_MVPA_ACT_ALL <- function(input,output,is.csvout=T, ID.nr=NULL, separator=",", dec=".",save_as_xlsx=FALSE, MVPA_cutoff=23000, bystep=FALSE, PrintMode=2)
  # Create MVPA-Bout table
{
  Sys.time()
  tab <- read_activPAL(inputpath = input)
  Sys.time()
  MVPA_addresses_tab <- MVPA_detect_transition(tab, MVPA_cutoff=MVPA_cutoff, bystep=bystep,PrintMode=PrintMode)
...
MVPA_detect_transition <- function(ActivPaltab, MVPA_cutoff, bystep=FALSE,PrintMode=2)
...
################ Event Detection ###################
CreateEvents <- function(AccTab, CatVar,id)
{
...
EventTabSupTab <- function(EventTab,EventName="")
{
  SupportTab <- NULL
  ids <- unique(EventTab[,1])
  for(ID in ids)
  {
    EvStart <- NULL
    EvEnd <- NULL
    IdEventTab <- EventTab[which(EventTab[,1]==ID),]
    print(paste("ID:",ID," Anzahl Events: ",nrow(IdEventTab),sep=""))
    for(i in 1:nrow(IdEventTab))
    {
      ...
      {
        EvStart <- c(EvStart,IdEventTab[i,2])
        EvEnd <- c(EvEnd,IdEventTab[i,3])
      }
    }
    EvStart <- as.POSIXct(EvStart,tz=getOption("tz"),origin = "1970-01-01")
    EvEnd <- as.POSIXct(EvEnd,tz=getOption("tz"),origin = "1970-01-01")
    print(EvStart)
    print(EvEnd)
    unq.Dte <- unique(as.Date(EvStart))
    print(unq.Dte)
    maxEventsPerDay <- 0
    for(dte in unq.Dte)
    {
      NumEv <- length(which(as.Date(EvStart)==dte))
    ...
    {
      EventSubTab <- matrix(data="",nrow=length(unq.Dte),ncol=(3))
    }
    else
    {
      EventSubTab <- matrix(data="",nrow=length(unq.Dte),ncol=(3*maxEventsPerDay))
    }
    for(dteNr in 1:length(unq.Dte))
    {
      Events <- which(as.Date(EvStart,tz=getOption("tz"))==unq.Dte[dteNr])
      for(EventNr in 1:length(Events))
      {
        EventSubTab[dteNr,((3*EventNr)-2)] <- format(EvStart[Events[EventNr]],"%H:%M:%S")
        EventSubTab[dteNr,((3*EventNr)-1)] <- format(EvEnd[Events[EventNr]],"%H:%M:%S")
  ...
  if(ncol(SupportTab)>=3)
  {
    for(i in 3:ncol(SupportTab))
    {
      if(length(which(SupportTab[,i]=="#NV"))>0)
      {
        SupportTab[which(SupportTab[,i]=="#NV"),i] <- ""
      }
    }
  }
  ...
  # {
  #   nr <- entry/3
  #   SupportTab[,entry] <- as.POSIXct(SupportTab[,entry],format="%Y-%m-%d %H:%M:%S")
  #   SupportTab[,(entry+1)] <- as.POSIXct(SupportTab[,(entry+1)],format="%Y-%m-%d %H:%M:%S")
...
calcVM <- function(acc.tab,colNrs=c(1,2,3))
# create vector magnitude from 3 channels
{
  x <- numeric(length = nrow(acc.tab))
  for(i in 1:nrow(acc.tab))
  {
    x[i] <- sqrt(sum((acc.tab[i,colNrs]**2)))
  }
  return(x)
}
...
select.window <- function(tab,i,window.size=60)
#return selected Window from table, just for sliding windows
{
  half.window <- window.size/2
  window.start <- i-ceiling(half.window)+1
  window.end <- i+floor(half.window)
  if(window.size > nrow(tab))
  {
    stop("window size bigger than table")
  }
  
  if(window.start <= 0)
  {
    window.end <- window.end+(1-window.start)
    window.start <- 1
  }
  
  if(window.end > nrow(tab))
  {
    window.start <- window.start-(window.end-nrow(tab))
    window.end <- nrow(tab)
  }
  return(c(window.start,window.end))
}

NwNaiv <- function(Acctab,varname,epochlength.min=1,null.thresh=0,window.size=60)
{
  window.size <- (window.size/epochlength.min)
  colnr <- which(colnames(Acctab) == varname)
  nw_naiv <- numeric(length = nrow(Acctab))
  for(i in 1:nrow(Acctab))
  {
    window.interval <- select.window(Acctab,i=i ,window.size=window.size)
    acc.window <- Acctab[window.interval[1]:window.interval[2],]
    zeros <- which(acc.window[,colnr] <= null.thresh)
    if(length(zeros) == window.size)
    {
      nw_naiv[window.interval[1]:window.interval[2]] <- rep.int(1,times = window.size)
    }
  }
  return(nw_naiv)
}

NwChoi <- function(acc.tab,varname,epochlength.min=1,null.thresh=0,window.size=90, unitercp.min=30,spike.min=2)
{
  colnr <- which(colnames(acc.tab) == varname)
  nw_choi <- numeric(length = nrow(acc.tab))
  for(i in 1:nrow(acc.tab))
  {
    window.interval <- select.window(acc.tab,i=i ,window.size=(window.size/epochlength.min))
    acc.window <- acc.tab[window.interval[1]:window.interval[2],]
    zeros <- which(acc.window[,colnr] <= null.thresh)
    
    if(length(which(acc.window[,colnr] > null.thresh))>0)
    {
      max.interc <- max(which(acc.window[,colnr] > null.thresh))
      min.interc <- min(which(acc.window[,colnr] > null.thresh))
      if(max.interc>0)
      {
        if(((max.interc-min.interc+1)*epochlength.min)<spike.min)
        {
          if((min.interc*epochlength.min) > unitercp.min)
          {
            if(((nrow(acc.window)-max.interc)*epochlength.min) > unitercp.min)
            {
              nw_choi[window.interval[1]:window.interval[2]] <- rep.int(1,times = (window.size/epochlength.min))
            }
          }
        }
      }
...
NwTroiano <- function(acc.tab,varname,epochlength.min=1,null.thresh=0, window.size=60, maxcount.thresh=100, spike.min=2)
{
  window.rest <- window.size-spike.min
  colnr <- which(colnames(acc.tab) == varname)
  nw_troi <- numeric(length = nrow(acc.tab))
  for(i in 1:nrow(acc.tab))
  {
    window.interval <- select.window(acc.tab,i=i ,window.size=(window.size/epochlength.min))
    acc.window <- acc.tab[window.interval[1]:window.interval[2],]
    if(max(acc.window[,colnr], na.rm = FALSE) <= maxcount.thresh)
    {
      under100 <- which(acc.window[,colnr] <= maxcount.thresh & acc.window[,colnr] > null.thresh)
      if(length(under100) <= (spike.min/epochlength.min))
      {
        zeros <- which(acc.window[,colnr] <= null.thresh)
        if(length(zeros) >= (window.rest/epochlength.min))
        {
          nw_troi[window.interval[1]:window.interval[2]] <- rep.int(1,times = (window.size/epochlength.min))
        }
      }
    }
  }
...
CalcNw <- function(AccPath,Method="naiv",separator=",", dec=".",null.thresh=400,window.size=90,unitercp.min=30,spike.min=2, maxcount.thresh=1000, ValuesbyDefault=TRUE)
{
  if(ValuesbyDefault)
  {
    if(Method=="naiv")
  ...
  accfiles <-paste(AccPath,list.files(path=AccPath, pattern="\\.csv$"),sep="")
  EventTab <- NULL
  print(paste("Predict NW in ",length(accfiles), " files at ",Sys.time(),sep=""))
  for(Accfile in accfiles)
  {
    id <- as.numeric(as.character(substr(basename(Accfile),1,5)))
    acc.tab <- read.table(Accfile,header=TRUE,sep=separator,dec=dec)
    #acc.tab <- data.matrix(acc.tab)
    VM <- calcVM(acc.tab,c(9,10,11))
    acc.tab <- cbind.data.frame(acc.tab,VM)
    nonw.vec <- NULL
    
    ### the different Methdos for detection ###
    if(Method=="naiv")
    {
      nonw.vec <- NwNaiv(Acctab=acc.tab,varname="VM",epochlength.min=0.25,
                         null.thresh=null.thresh,window.size=window.size)
    }
    if(Method=="choi")
    {
      nonw.vec <- NwChoi(acc.tab=acc.tab,varname="VM",epochlength.min=0.25,
                         null.thresh=null.thresh,window.size=window.size,unitercp.min=unitercp.min,spike.min=spike.min)
    }
    if(Method=="Troiano")
    {
      nonw.vec <- NwTroiano(acc.tab,varname,epochlength.min=0.25,
                            null.thresh=null.thresh, window.size=window.size, maxcount.thresh=maxcount.thresh, spike.min=spike.min)
    }
    if(Method=="RF")
    {
      stop("not yet implemented: Method Random Forest")
    }
    ####
    
    acc.tab <- cbind.data.frame(acc.tab,VM,nonw.vec)
    n_date <- as.POSIXct((acc.tab[,1] + 693960 - 719529)*86400, origin = "1970-01-01", tz=getOption("tz"))
    acc.tab <- cbind.data.frame(n_date,acc.tab[,(2:ncol(acc.tab))])
    colnames(acc.tab)[1] <- "DateTime"
    colnames(acc.tab)[ncol(acc.tab)] <- "NWcat"
    if(is.null(EventTab))
    {
      EventTab <- CreateEvents(acc.tab,"NWcat",id)
    }
    else
    {
      EventTab <- rbind.data.frame(EventTab,CreateEvents(acc.tab,"NWcat",id))
    }
    print("------------------------------------------------------")
    print(paste("Prediction for ID:",id," complete at ",Sys.time(),sep=""))
    print("------------------------------------------------------")
  }
  NwTab <- EventTabSupTab(EventTab,paste("auto: ",Method,sep=""))
  print(paste("Nonwear Table Format created for Support Files at ",Sys.time(),sep=""))
  return(NwTab)
}
  ...
  # Merging created analysis table to one big anlysis-table for output
{
  ...
  if(!is.null(NW.tab))
  {
    if(length(dim(NW.tab))<2)
    {
      if(length(which(DateDayTab[,3]==NW.tab[1]))<1)
      {
        NW.tab <- NULL
      }
    }
    else
    {
      ...
      {
        for(i.nw in 1:nrow(NW.tab))
        {
          if(length(which(DateDayTab[,3] == NW.tab[i.nw,1])) < 1)
          {
            nwDel <- c(nwDel,i.nw)
          }
        }
  ...
  H24.dels <- which(H24_functional[,2] >= 1440 )
  ...
  H24.delZero <- which(H24_functional[,2] <= 0)
  if(nrow(H24.BoutSum)<nrow(H24_functional) && length(H24.delZero) > 0)
  {
    NW.del <- NULL
    for(dels in H24.delZero)
    {
      NW.del <- c(NW.del,DateDayTab[which(DateDayTab[,2] == dels),3])
    }
    if(nrow(H24_functional) == nrow(sleep.tab))
    {
      sleep.tab <- sleep.tab[-H24.delZero,]
    }
    if(!is.null(DateDayTab))
    {
      DateDayTab <- DateDayTab[-H24.delZero,]
      if(is.null(nrow(NW.tab)))
      {
        if(NW.del == NW.tab[1])
        {
    ...
    H24_functional <- H24_functional[-H24.delZero,]
    H24.dels <- c(H24.dels,H24.delZero)
  }
...
generate_supportfiles <- function(Acc.input, Domain.out=NULL, NW.out=NULL, id.length,id.start=1, separated=TRUE, separator=",", dec=".", save_as_xlsx=FALSE,
                                  Slp.out=NULL, NwMethod="default",null.thresh=400,window.size=90,unitercp.min=30,spike.min=2, maxcount.thresh=1000, ValuesbyDefault=TRUE)
  # function to create nonwear-time and domain-supportfiles for all ActivPal-datasets 
{
  ...
  else
  {
    ...
    if(NwMethod=="default" && !is.null(NW.path))
    {
      for(i in 1:length(acc.list))
      {
        acc.file <- get_filename(acc.list[i])
    ...
    else
    {
      if(!is.null(NW.path))
      {
        NWFile <- CalcNw(AccPath=Acc.input,Method=NwMethod,separator=separator, dec=dec,
                         null.thresh=null.thresh,window.size=window.size,unitercp.min=unitercp.min,spike.min=spike.min, 
                         maxcount.thresh=maxcount.thresh, ValuesbyDefault=ValuesbyDefault)
      }
    }
...
batch_start <- function(Acc.input,id.length,output,Domain.out=NULL,NW.out=NULL,separated.csv=FALSE,JoinedDomains=NULL,JoinedNWs=NULL,
                        ...
                        RunSedBouts=TRUE,RunMVPA_Bouts=TRUE, SedBoutMasterTab=NULL, 
                        NwMethod="default",null.thresh=400,window.size=90,unitercp.min=30,spike.min=2, maxcount.thresh=1000, ValuesbyDefault=TRUE)
  # controll function to start generate_supportfiles or batch_run with the possible option of running Individual_Avg_data
{
  ...
  else
  {
    ...
    {
      suppressWarnings( 
        z <- MVPA_Batch(input=Acc.input,output=output,ACT.ALL.out=ACT.ALL.out,domainCsv=Domain.out,NW_path=NW.out,
                        early=early, late=late ,max.break=max.break,min.sleeplength=min.sleeplength,id.length=id.length
                        ,separated.csv=separated.csv, JoinedDomains=JoinedDomains,JoinedNWs=JoinedNWs, separator=separator, dec=dec
                        , run.from=run.from, run.to=run.to, fill_sleepTab=fill_sleepTab,day.begin=day.begin, time.unit=time.unit, mvpa_steps_cutoff=mvpa_steps_cutoff, 
                        WriteMastersheet_perID=WriteMastersheet_perID, is.xlsx=is.xlsx, sheetIndex=sheetIndex, calcBySteps=calcBySteps, mvpa_count_thresh=mvpa_count_thresh,save_as_xlsx=save_as_xlsx
                        , non.stop=non.stop,id.asnumeric=id.asnumeric,PrintMode=PrintMode)
      )
    }
    else
    {
    ...
    if(calcIndivualAvg)
    {
      ...
      {
        print("---------------------------")
        print(paste("generate individual averages for SED Mastersheet started at",Sys.time(),sep = " ... "))
      }
      ...
      {
        if(is.null(SedBoutMasterTab))
        {
          stop("Individual averages could not be calculated: No SED Bout Mastersheet was calculated")
        }
        else
        {
          y <- Individual_Avg_data(SedBoutMasterTab,AVG_Filepath, separator=separator, dec=dec,ignore.firstday=ignore.firstday, ignore.lastday=ignore.lastday,save_as_xlsx=save_as_xlsx)
        }
      }
      if(PrintMode>0)
      {
        print(paste("generate individual averages for SED Mastersheet finished at",Sys.time(),sep = " ... "))
        print("---------------------------")
      }
    }
    return(list(SED_MasterSheet=x,ID_Averages=y,MVPA_MasterSheet=z))
  }
}
...
MVPASheet_run <- function(accfile,output,ACT.ALL.out=FALSE,id.length=8,id.start = 1,id.asnumeric=FALSE,
                          ...
                          is.xlsx=FALSE,sheetIndex=1,save_as_xlsx=FALSE,PrintMode=2)
{
  if(id.asnumeric)
  {
  ...
  if(PrintMode > 0)
  {
    print(paste("processing ID: ",ID.nr,sep=""))
  }
  
  mvpaTab <- FN_CPAHR_MVPA_ACT_ALL(input=accfile,output,is.csvout=ACT.ALL.out, ID.nr=ID.nr, separator=separator, dec=dec,save_as_xlsx=save_as_xlsx, MVPA_cutoff=MVPA_cutoff, bystep=calcBySteps,PrintMode=PrintMode)
...
MVPASheet.lapply <- function(accfile,output,ACT.ALL.out,domainCsv=NULL,NW_path,early="20:00:00", late="12:00:00",max.break=15,min.sleeplength=45,id.length,id.start=1,separated.csv=TRUE, JoinedDomains=NULL
                             ,JoinedNWs=NULL, separator=",", dec=".",fill_sleepTab=FALSE, day.begin="00:00:00",time.unit="hours",mvpa_steps_cutoff=25,WriteMastersheet=FALSE, is.xlsx=FALSE, sheetIndex=1,
                             calcBySteps=TRUE, mvpa_count_thresh=2296, save_as_xlsx=FALSE, non.stop=TRUE,id.asnumeric=FALSE, PrintMode=2) 
  # Loads supportfiles and starts complete_run function
{
  if(calcBySteps)
  {
  ...
  if(is.null(NW_path))
  {
    ...
    if(length(ID.DomFile)<=1)
    {
      ...
      MVPASheet <- MVPASheet_run(accfile,output,ACT.ALL.out=ACT.ALL.out,id.length=id.length,id.start = id.start,id.asnumeric=id.asnumeric,
                                 ...
                                 is.xlsx=is.xlsx,sheetIndex=sheetIndex,save_as_xlsx=save_as_xlsx, PrintMode=PrintMode)
    }
    else
    {
      print(paste("unequal 1 DomainFiles found for the ID",acc.ID ,sep=": "))
...
MVPA_Batch <- function(input,output,ACT.ALL.out=FALSE,domainCsv=NULL,NW_path=NULL,early="20:00:00", late="12:00:00",max.break=15,
                       min.sleeplength=45,id.length,id.start=1,separated.csv=TRUE, JoinedDomains=NULL,JoinedNWs=NULL, 
                       separator=",", dec=".", run.from=1, run.to=NULL,fill_sleepTab=FALSE, day.begin="00:00:00",time.unit="hours", mvpa_steps_cutoff=25, WriteMastersheet_perID=FALSE
                       , is.xlsx=FALSE, sheetIndex=1, calcBySteps=TRUE, mvpa_count_thresh=2296,save_as_xlsx=FALSE, non.stop=TRUE,id.asnumeric=FALSE, PrintMode=2)
  # Runs MasterSheet.lapply for all Ids and merges the Mastersheetfiles together
{
  if(is.null(domainCsv))
  {
  ...
  if(PrintMode > 0)
  {
    print(paste("Create MVPA Bouts for ",length(acc.list)," files",sep=""))
    print(paste("started at ",Sys.time(),sep=""))
    print("---------------------------")
  }
  for(i in 1:length(acc.list))
  {
    if(i == 1)
    {
      Master.Sheet <- MVPASheet.lapply(acc.list[i],output=output,ACT.ALL.out=ACT.ALL.out,domainCsv=domainCsv,
                                       ...
                                       ,time.unit=time.unit, mvpa_steps_cutoff=mvpa_steps_cutoff, WriteMastersheet=WriteMastersheet_perID
                                       , is.xlsx=is.xlsx, sheetIndex=sheetIndex, calcBySteps=calcBySteps, mvpa_count_thresh=mvpa_count_thresh,
                                       save_as_xlsx=save_as_xlsx, non.stop=non.stop,id.asnumeric=id.asnumeric,PrintMode=PrintMode)
    }
    else
    {
    ...
    {
      print("---------------------------")
      print(paste("MVPA Bouts processed for ",i," of ",length(acc.list)," files at ",Sys.time(),sep=""))
      print("---------------------------")
    }
  }
  colnames(Master.Sheet)[1] <- c("ID")
